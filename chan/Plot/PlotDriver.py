...
def set_x_tick(ax, x_limits, tick, x_tick_num: int, tick_labels=True):
    assert x_tick_num > 1
    ...
    ax.set_xticks(
        range(
            x_limits[0],
            x_limits[1],
            max([1, int((x_limits[1] - x_limits[0]) / float(x_tick_num))]),
        )
    )
    if tick_labels:
...
def create_figure(
    plot_macd: Dict[KL_TYPE, bool], figure_config, lv_lst: List[KL_TYPE]
) -> Tuple[Figure, Dict[KL_TYPE, List[Axes]]]:
    """
    返回：
        - Figure
    ...
    figure, axes = plt.subplots(
        sub_pic_cnt, 1, figsize=(w, total_h), gridspec_kw={"height_ratios": gridspec_kw}
    )
    try:
        axes[0]
...
class CPlotDriver:
    def __init__(
        ...
        srange_begin = 0
        assert (
            slv_seg_cnt is None or slv_bi_cnt is None
        ...
        for meta, lv in zip(plot_metas, self.lv_lst):  # type: ignore
            ...
            ax.set_title(
                f"{chan.code}/{lv.name.split('K_')[1]}",
                fontsize=16,
                loc="left",
                color="r",
            )
            ...
            self.DrawElement(
                plot_config[lv], meta, ax, lv, plot_para, ax_macd, x_limits
            )
    ...
    def GetRealXrange(self, figure_config, meta: CChanPlotMeta):
        ...
        if x_range != 0:
            assert (
                bi_cnt == 0 and seg_cnt == 0 and x_begin_date == 0
            ), "x_range/x_bi_cnt/x_seg_cnt/x_begin_date can not be set at the same time"
            return x_range
        if bi_cnt != 0:
            assert (
                x_range == 0 and seg_cnt == 0 and x_begin_date == 0
            ), "x_range/x_bi_cnt/x_seg_cnt/x_begin_date can not be set at the same time"
            X_LEN = meta.klu_len
        ...
        if seg_cnt != 0:
            assert (
                x_range == 0 and bi_cnt == 0 and x_begin_date == 0
            ), "x_range/x_bi_cnt/x_seg_cnt/x_begin_date can not be set at the same time"
            X_LEN = meta.klu_len
        ...
        if x_begin_date != 0:
            assert (
                x_range == 0 and bi_cnt == 0 and seg_cnt == 0
            ), "x_range/x_bi_cnt/x_seg_cnt/x_begin_date can not be set at the same time"
            x_range = 0
    ...
    def draw_klu(
        self, meta: CChanPlotMeta, ax: Axes, width=0.4, rugd=False, plot_mode="kl"
    ):
    ...
    def draw_bi(
        ...
        for bi_idx, bi in enumerate(meta.bi_list):
            ...
            if show_num and bi.begin_x >= x_begin:
                ax.text(
                    (bi.begin_x + bi.end_x) / 2,
                    (bi.begin_y + bi.end_y) / 2,
                    f"{bi.idx}",
    ...
    def draw_seg(
        ...
        for seg_idx, seg_meta in enumerate(meta.seg_list):
            ...
            if seg_meta.is_sure:
                ax.plot(
                    [seg_meta.begin_x, seg_meta.end_x],
            ...
            if disp_end:
                if seg_idx == 0:
                    ax.text(
                        ...
                        color=end_color,
                        verticalalignment=(
                            "top" if seg_meta.dir == BI_DIR.UP else "bottom"
                        ),
                ...
                ax.text(
                    ...
                    color=end_color,
                    verticalalignment=(
                        "top" if seg_meta.dir == BI_DIR.DOWN else "bottom"
                    ),
                    horizontalalignment="center",
                )
    ...
    def draw_zs(
        ...
        for zs_meta in meta.zs_lst:
            ...
            for sub_zs_meta in zs_meta.sub_zs_lst:
                ax.add_patch(
                    Rectangle(
                        (sub_zs_meta.begin, sub_zs_meta.low),
                        sub_zs_meta.w,
                        sub_zs_meta.h,
                        fill=False,
                        color=color,
    ...
    def draw_segzs(
        self, meta: CChanPlotMeta, ax: Axes, color="red", linewidth=10, sub_linewidth=4
    ):
        linewidth = max(linewidth, 2)
        x_begin = ax.get_xlim()[0]
    ...
    def draw_macd(self, meta: CChanPlotMeta, ax: Axes, x_limits, width=0.4):
        ...
        _bar = ax.bar(x_idx, macd_bar, color="g", width=width)
        for idx, macd in enumerate(macd_bar):
            if macd < 0:
                _bar[idx].set_color("r")

        # 检查 y_min 和 y_max 是否相同
        if y_min == y_max:
            # 如果相同，自动扩展 y 轴范围，例如增加 1%
            epsilon = y_min * 0.01 if y_min != 0 else 1  # 避免 y_min 为 0 时的情况
            y_min -= epsilon
            y_max += epsilon
            # print(f"Adjusted y-axis limits: y_min={y_min}, y_max={y_max}")
    ...
    def draw_mean(self, meta: CChanPlotMeta, ax: Axes):
        ...
        for cmap_idx, T in enumerate(Ts):
            ...
            ax.plot(
                range(len(mean_arr)), mean_arr, c=cmap(cmap_idx), label=f"{T} meanline"
            )
        # ax.legend()
    """
    ...
    def draw_mean(
        ...
        参数:
            meta (CChanPlotMeta): 包含均线信息的元数据。
            ax (Axes): Matplotlib 轴对象，用于绘制图形。
            show_legend (bool): 是否显示图例，默认为 False 不显示。
            slope_color (bool): 是否根据斜率改变均线颜色，默认为 False 不改变。
                                如果开启，根据当前均线值与上一个值的比较，颜色为绿色或红色。
        """
        mean_lst = [klu.trend[TREND_TYPE.MEAN] for klu in meta.klu_iter()]
        Ts = list(mean_lst[0].keys())

        # 计算周期（T）的大小，用于决定线条宽度
        sorted_Ts = sorted(Ts)  # 按周期从小到大排序
        min_width, max_width = 1, 5  # 设定线条宽度范围
        width_map = {
            T: min_width + idx for idx, T in enumerate(sorted_Ts)
        }  # 动态分配宽度

        if not slope_color:
            cmap = plt.cm.get_cmap("hsv", max([10, len(Ts)]))  # 使用颜色映射

        for cmap_idx, T in enumerate(Ts):
            mean_arr = [mean_dict[T] for mean_dict in mean_lst]
            line_width = width_map[T]  # 根据周期大小获取线条宽度

            if slope_color:
                # 使用斜率颜色：绿色表示上升，红色表示下降
                ...
                # 根据斜率确定每个线段的颜色
                colors = [
                    "green" if y2 > y1 else "red" for y1, y2 in zip(y[:-1], y[1:])
                ]

                # 创建 LineCollection 并添加到轴上
                lc = LineCollection(segments, colors=colors, linewidth=line_width)
                ax.add_collection(lc)

                # 为图例创建代理艺术家（Proxy Artist）
                if show_legend:
                    overall_trend = "上升" if y[-1] > y[0] else "下降"
                    legend_color = "green" if y[-1] > y[0] else "red"
                    proxy = plt.Line2D(
                        [0],
                        [0],
                        color=legend_color,
                        linewidth=line_width,
                        label=f"{T} meanline ({overall_trend})",
                    )
                    ax.add_line(proxy)
            else:
                # 使用颜色映射设置线条颜色
    ...
    def draw_boll(
        ...
        draw_mid: bool = False,  # 新增参数，默认不绘制中轨线
        alpha=0.6,
        linewidth=2,
    ):
        """
        ...
        参数:
            meta (CChanPlotMeta): 包含布林带数据的元数据。
            ax (Axes): Matplotlib 轴对象，用于绘制图形。
            mid_color (str): 中轨线颜色，默认为 "orange"。
            up_color (str): 上轨线颜色，默认为 "lightblue"。
            down_color (str): 下轨线颜色，默认为 "lightblue"。
            draw_mid (bool): 是否绘制中轨线，默认为 False。
        """
        x_begin = int(ax.get_xlim()[0])  # 获取 x 轴起始位置
        try:
        ...
        # 绘制中轨线（根据 draw_mid 参数决定是否绘制）
        if draw_mid:
            ax.plot(
                range(x_begin, x_begin + len(ma)),
                ma,
                c=mid_color,
                label="Middle Band",
                linewidth=linewidth,
        ...
        # 绘制上轨线和下轨线，线宽为2，透明度为50%
        ax.plot(
            range(x_begin, x_begin + len(up)),
            up,
            c=up_color,
            label="Upper Band",
            linewidth=linewidth,
        ...
        ax.plot(
            range(x_begin, x_begin + len(down)),
            down,
            c=down_color,
            label="Lower Band",
            linewidth=linewidth,
    ...
    def draw_marker(
        ...
        for date, marker in new_marker.items():
            ...
            ax.arrow(
                ...
                0,
                (arror_len - arrow_h)
                * _dir,  # 箭头的长度实际上是arror_len+arrow_h，所以要减去，顺便减去半个箭头防止和K线重叠
                head_width=arrow_w,
                head_length=arrow_h,
                color=color,
            )
    ...
    def draw_kdj(
        ...
        ax.plot(
            range(x_begin, x_end),
            [x.k for x in kdj][x_begin:x_end],
            c=k_color,
            label="K",
        )
        ax.plot(
            range(x_begin, x_end),
            [x.d for x in kdj][x_begin:x_end],
            c=d_color,
            label="D",
        )
        ax.plot(
            range(x_begin, x_end),
            [x.j for x in kdj][x_begin:x_end],
            c=j_color,
            label="J",
        )
        ax.legend()
        ...
        for klu in meta.klu_iter():
            ...
            for demark_idx in klu.demark.get_setup():
                ...
                self.draw_demark_begin_line(
                    ax, begin_line_color, plot_begin_set, begin_line_style, demark_idx
                )
                txt_instance = ax.text(
                    ...
                    color=setup_color,
                    verticalalignment=(
                        "top" if demark_idx["dir"] == BI_DIR.DOWN else "bottom"
                    ),
                    horizontalalignment="center",
                )
                if demark_idx["dir"] == BI_DIR.DOWN:
                    under_bias += (
                        getTextBox(ax, txt_instance).height
                        if demark_idx["dir"] == BI_DIR.DOWN
                        else 0
                    )
                else:
            ...
            for demark_idx in klu.demark.get_countdown():
                box_bias = (
                    ...
                    and demark_idx["idx"] == CDemarkEngine.MAX_COUNTDOWN
                    else 0
                )
                txt_instance = ax.text(
                    klu.idx,
                ...
                if demark_idx["idx"] == CDemarkEngine.MAX_COUNTDOWN:
                    txt_instance.set_bbox(
                        dict(
                            facecolor=max_countdown_background,
                            edgecolor=max_countdown_background,
                            pad=0,
                        )
                    )
                if demark_idx["dir"] == BI_DIR.DOWN:
